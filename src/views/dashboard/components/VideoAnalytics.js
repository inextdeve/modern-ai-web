...
const VideoAnalytics = () => {
    ...
    const enabledCameras = useSelector(state => state.cameras.items.filter((camera) => camera.enabled));
    ...
    useEffect(() => {
        ...
        setViewedCamera(enabledCameras.filter(camera => camera.analytics[selectedModule] === true))
    ...
    return (
        ...
        <DashboardCard title="Video Analytics" action={
            <Select
            ...
            >
                ...
                {analyticsModules.map((module) => (<MenuItem key={module.id} value={module.id}>{module.name}</MenuItem>))}

            </Select>
        }>
            <Grid container spacing={4}>
                {viewedCameras.map((camera) => (
                    <Grid item xs={6}>
